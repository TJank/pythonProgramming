# Vignere Cipher
# Divide and Conquer
# 1. Fix key to be the smae length as a message
...
# 4. Encode by mapping a pair (X, Y) -> Z where X is a letter from the key,
# Y is a letter from the message and Z can be found in the Vignere Square

import string

def create_keyword(keyword, n):
    """
    Function to creat an appropriate keyword to be used in a Vignere Cipher encryption system
    ...
    if len(keyword) < n:    # new_keyword needs to grow up to size n
        for i in range(n):
            if idx >= len(keyword):     # reset accumulator now before you do something important
                idx = 0
